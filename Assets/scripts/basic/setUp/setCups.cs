...
public class setCups : MonoBehaviour
{
    ...
    public void MakeCups()
    {
        ...
        float ballYPos = -1.8f-cupHeight*.25f;
        ...
        for (int i = 0; i < cupsAmt; i++){
            ...
            cup.transform.localScale = new Vector2(cupWidth,cupWidth)/cupSpriteSize;
    ...
    public List<Vector2> getCupPositions()
    {
        ...
        for (int i = 0; i < cupsAmt; i++){
            positions.Add(new Vector2(-border+cupDistance/2f+cupDistance*i,setY));
        }
        return positions;
    }
}
