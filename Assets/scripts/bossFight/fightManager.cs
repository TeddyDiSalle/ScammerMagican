...
public class fightManager : MonoBehaviour
{
    ...
    void Start()
    {
        //fightReady = true;//temp for testing
    }
    ...
    public void checkReady(int level)
    {
        fightReady = level>=this.level;
    }
    ...
    IEnumerator moveLocalPos0(Transform closestCup,bool final = false)
    {
        ...
        while ((!final&&Mathf.Abs(ball.localPosition.x)>0.1f)||(final&&Vector2.Distance(ball.localPosition,Vector2.zero)>0.1f))
        {
            speed += acceleration*Time.deltaTime;
            if (!final)
                ball.localPosition = new Vector2(Mathf.MoveTowards(ball.localPosition.x, 0f, speed * Time.deltaTime),ball.localPosition.y);
            else
                ball.localPosition = Vector2.MoveTowards(ball.localPosition, Vector2.zero, speed * Time.deltaTime);
        ...
        List<Vector2> positions = gameSetter.getCupPositions();
            foreach (Vector2 position in positions){
                ...
                foreach (Transform thisCup in lowerCup.cups){
                    //Debug.Log(cup+":"+(Mathf.Abs(cup.position.x-position.x)<=.5f));
                ...
                if (spotFilled)
                    continue;
                yield return StartCoroutine(GoToPos.MoveCoroutine(resetCup,new Vector2(position.x,resetCup.position.y-lowerCup.lowerAmt),lowerCup.duration));
    ...
    {
        yield return StartCoroutine(GoToPos.MoveCoroutine(cup,cup.position+new Vector3(0f,lowerCup.lowerAmt*1f,0f),lowerCup.duration));
        //yield return StartCoroutine(GoToPos.MoveCoroutine(cup,cup.position+new Vector3(0f,lowerCup.lowerAmt*-1f,0f),lowerCup.duration));
